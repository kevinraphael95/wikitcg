...
async function fetchYGOEffect(category, subtype, attr, race, level, h) {
  ...
  try {
    ...
    for (const card of data.data) {
      ...
      const parts = card.desc.split(/(?<=[.!])\s+|\n/).map(s => s.trim()).filter(s => s.length > 25 && s.length < 200 && !/["«»""][\w\s'-]{3,35}["»""]/.test(s));
...
function mtgAbility(category, subtype, desc, color, h) {
  ...
  if (category === 'place') {
    if (/île|océan|mer |lac |fleuve|rivière/.test(d)) return `{T} : Ajoutez ${m[color]||'{U}'} à votre réserve de mana.\n{T} : Piochez une carte, puis défaussez-vous d'une carte.`;
    if (/montagne|volcan/.test(d)) return `{T} : Ajoutez ${m[color]||'{R}'} à votre réserve de mana.\n{T}, Sacrifiez ~ : Cette carte inflige 2 blessures à n'importe quelle cible.`;
    if (/forêt|jungle|parc/.test(d)) return `{T} : Ajoutez ${m[color]||'{G}'} à votre réserve de mana.\n{T} : Cherchez une Forêt de base dans votre bibliothèque, mettez-la sur le champ de bataille.`;
    if (/château|palais|temple|cathédrale/.test(d)) return `{T} : Ajoutez ${m[color]||'{W}'} à votre réserve de mana.\nLes créatures légendaires que vous contrôlez gagnent +1/+1.`;
    return `{T} : Ajoutez ${m[color]||'{C}'} à votre réserve de mana.`;
  }
  if (category === 'person') {
    if (/roi |reine |emperor|impératrice|pharaon/.test(d)) return `Au début de chaque combat, les autres créatures que vous contrôlez gagnent +1/+1 jusqu'à la fin du tour.\nVous pouvez invoquer des créatures légendaires sans payer leur coût de mana.`;
    if (/général|amiral|maréchal/.test(d)) return `Quand ~ entre sur le champ de bataille, cherchez dans votre bibliothèque une carte Créature de force 2 ou moins et mettez-la sur le champ de bataille.\nLes créatures que vous contrôlez ont la Célérité.`;
    if (/physicien|scientifique|inventeur|ingénieur/.test(d)) return `{2}, {T} : Créez un jeton Artefact 0/0 incolore avec deux marqueurs +1/+1.\nLes Artefacts que vous contrôlez ont la Hexproof.`;
    if (/philosophe|écrivain|auteur|historien/.test(d)) return `Quand ~ entre sur le champ de bataille, piochez deux cartes.\nVos sorts coûtent {1} de moins à lancer.`;
    if (/prêtre|moine|pape|évêque|saint/.test(d)) return `Lien de vie.\nChaque fois que vous gagnez des points de vie, mettez un marqueur +1/+1 sur ~.`;
    if (/assassin|espion/.test(d)) return `Discrétion.\nChaque fois que ~ inflige des blessures de combat à un joueur, ce joueur défausse une carte.`;
    if (/sportif|athlète|boxeur|champion/.test(d)) return `Célérité. Premier combat.\nChaque fois que ~ attaque, il gagne +1/+0 pour chaque autre créature attaquante.`;
...
function buildMTG(summary, cats) {
  ...
  return { name: (summary.title || '').slice(0, 27), color, category: cat, subtype, kws, ability, pt, mana, rarity, typeStr, flavor, image: summary.thumbnail?.source || null, url: summary.content_urls?.desktop?.page || '#' };
}
...
function renderMTG(c) {
  ...
  if (c.pt) { ptEl.textContent = `${c.pt.p}/${c.pt.t}`; ptEl.classList.add('show'); }
  else { ptEl.classList.remove('show'); }
}
...
function ygoAttr(category, desc) {
  const d = desc.toLowerCase();
  if (category === 'place') { if (/île|océan|mer |lac |fleuve/.test(d)) return 'WATER'; if (/montagne|volcan|désert/.test(d)) return 'FIRE'; return 'EARTH'; }
  if (category === 'animal') { if (/aquatique|marin|poisson|baleine|requin/.test(d)) return 'WATER'; if (/dragon|serpent|vulcain|feu/.test(d)) return 'FIRE'; if (/nocturne|chauve-souris|sombre/.test(d)) return 'DARK'; if (/aigle|faucon|oiseau/.test(d)) return 'WIND'; return 'EARTH'; }
  if (category === 'person') { if (/saint|divin|ange|pape|roi |reine/.test(d)) return 'LIGHT'; if (/assassin|tyran|dictateur|sombre/.test(d)) return 'DARK'; if (/militaire|guerrier|général|conquête/.test(d)) return 'FIRE'; if (/scientifique|inventeur|mage/.test(d)) return 'WIND'; return 'EARTH'; }
...
function ygoRace(category, subtype) {
  ...
  const map = { Dragon:'Dragon', Reptile:'Reptile', Araignée:'Insecte', Insecte:'Insecte', Oiseau:'Oiseau', Aquatique:'Poisson', Loup:'Bête', Félin:'Bête', Ours:'Bête', Bête:'Bête', Dinosaure:'Dinosaure', Singe:'Bête', Cheval:'Bête', Plante:'Plante', Noble:'Guerrier', Guerrier:'Guerrier', Mage:'Magicien', Prêtre:'Magicien', Sage:'Magicien', Espion:'Guerrier', Champion:'Guerrier', Artiste:'Magicien', Humain:'Guerrier' };
  return map[subtype] || 'Guerrier';
}

function ygoKind(category, desc, h) {
  const d = desc.toLowerCase();
  if (['event','work','concept','place','object'].includes(category)) { if (category === 'place') return 'spell'; if (category === 'event') return /piège|embuscade|trahison|surprise/.test(d) ? 'trap' : 'spell'; return h % 3 === 0 ? 'trap' : 'spell'; }
  if (category === 'animal' && /dragon/.test(d) && h % 4 === 0) return 'fusion';
  if (category === 'person' && /dieu|déesse|divin/.test(d)) return 'ritual';
  const r = h % 10;
  if (r === 0) return 'normal'; if (r === 1) return 'fusion'; if (r === 2) return 'synchro'; if (r === 3) return 'xyz';
  return 'effect';
}
...
function ygoBracket(kind, race, category, desc, h) {
  ...
  if (category === 'object') { const sub = objectSubtype(d); if (sub === 'Arme' || sub === 'Armure') return '[Magie — Équipement]'; return '[Magie — Normal]'; }
...
function ygoEffectFallback(category, desc, kind, h) {
  const d = desc.toLowerCase();
  if (category === 'place') { if (/île|océan|mer |lac |fleuve/.test(d)) return `Sort de Terrain.\nTant que cette carte est sur le Terrain, tous les monstres EAU gagnent 500 ATK/DEF.`; if (/montagne|volcan/.test(d)) return `Sort de Terrain.\nTant que cette carte est sur le Terrain, tous les monstres FEU gagnent 500 ATK.`; return `Sort de Terrain.\nTant que cette carte est sur le Terrain, tous vos monstres gagnent 300 ATK/DEF.`; }
...
async function buildYGO(summary, cats) {
  ...
  let eff = '';
  if (kind !== 'normal') {
    try { const sentences = await fetchYGOEffect(category, subtype, attr, race, level, h); if (sentences.length) eff = pickEffect(sentences, h); } catch {}
    if (!eff) eff = ygoEffectFallback(category, desc, kind, h);
  }
  const flavor = kind === 'normal' ? (summary.description || '') : '';
  return { name: (summary.title || '').slice(0, 28), kind, attr, level, race, stats, eff, flavor, bracket, category, isXYZ: kind === 'xyz', isLink: kind === 'link', linkN: kind === 'link' ? rng(1, 4, h) : null, serial: String(Math.abs(h)).padStart(8,'0').slice(0, 8), image: summary.thumbnail?.source || null, url: summary.content_urls?.desktop?.page || '#', emoji: ATTRS[attr] || '🌍' };
}
...
function renderYGO(c) {
  ...
  if (c.level && !c.isLink) {
    ...
    for (let i = 0; i < c.level; i++) { const s = document.createElement('span'); s.className = starClass; s.textContent = starChar; starsEl.appendChild(s); }
  } else if (c.isLink) {
    levelRow.style.display = '';
...
function setArt(img, ph, src, emoji) {
  ...
  } else { img.style.display = 'none'; ph.textContent = emoji; ph.style.display = 'flex'; }
}
...
function setLoading(on) {
  ...
  ['bGen','bRand','bDaily'].forEach(id => { const el = document.getElementById(id); if(el) el.disabled = on; });
}
